...
class RapportController extends Controller
{
    ...
    private function getVentesData($validated, $user)
    {
        $query = Vente::with(['produit', 'boutique.magasin'])
                      ->whereDate('date', '>=', $validated['date_debut'])
                      ->whereDate('date', '<=', $validated['date_fin']);
        ...
        $ventes = $query->orderBy('date')->get();

        // Calculer les totaux
        $data['ventes'] = $ventes;
        $data['totalVentes'] = $ventes->count();
        $data['totalCA'] = $ventes->sum('prix_total');
        $data['totalBenefice'] = $ventes->sum('benefice');
        ...
        $data['ventesParBoutique'] = $ventes->groupBy('boutique_id')
                                           ->map(function($group) {
                                               ...
                                               return [
                                                   ...
                                                   'ventes' => $group->count(),
                                                   'ca' => $group->sum('prix_total'),
                                                   'benefice' => $group->sum('benefice')
                                               ];
                                           });

        // Grouper par produit
        $data['ventesParProduit'] = $ventes->groupBy('produit_id')
                                          ->map(function($group) {
                                              $first = $group->first();
                                              return [
