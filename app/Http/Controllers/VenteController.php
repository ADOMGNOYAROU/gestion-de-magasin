...
class VenteController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($search) {
            $query->where(function($q) use ($search) {
                $q->whereHas('venteProduits.produit', function($subq) use ($search) {
        ...
        if ($date_debut) {
            $query->whereDate('date_vente', '>=', $date_debut);
        }
    ...
    public function store(Request $request)
    {
        ...
        if (empty($panier)) {
            if ($request->expectsJson()) {
                return response()->json([
                    'success' => false,
                    'message' => 'Le panier est vide. Ajoutez des produits avant de valider la vente.',
                ], 400);
            }
        ...
        try {
            ...
            if (!$paymentMethod) {
                throw new \Exception('Aucune méthode de paiement active trouvée. Veuillez configurer les méthodes de paiement.');
            }
            ...
            $successMessage = "Vente enregistrée avec succès ! Total : " . number_format($totalVente, 0, ',', ' ') . " FCFA | Bénéfice : " . number_format($totalBenefice, 0, ',', ' ') . " FCFA";

            if ($request->expectsJson()) {
                return response()->json([
                    'success' => true,
                    'message' => $successMessage,
                    'redirect_url' => route('ventes.index'),
                ]);
            }
